...
Dados *inicia_dados(char *EQUIPAMENTO, int idade);
Dados *insere_dados(Dados *dados, char *EQUIPAMENTO, int idade);
...
Dados *inicia_dados(char *EQUIPAMENTO, int idade) {
        ...
        novo->EQUIPAMENTO = (char *)malloc(strlen(EQUIPAMENTO)+1);
        strncpy(novo->EQUIPAMENTO, EQUIPAMENTO, strlen(EQUIPAMENTO)+1);
        novo->idade = idade;
        novo->proximo = NULL;
...
/* Percorre cada ponta comparando o EQUIPAMENTO com a chave. */
void busca_dados(Dados *dados, char *chave) {
        ...
        fprintf(stdout, "Cadastro:\n\n");
        for (; dados != NULL; dados = dados->proximo) {
                if (strcmp(chave, dados->EQUIPAMENTO) == 0) {
...
void insere(void) {
        ...
        EQUIPAMENTO = (char *)malloc(BUFFER);
        ...
        if (principal == NULL)
                principal = inicia_dados(EQUIPAMENTO, idade);
        else
                principal = insere_dados(principal, EQUIPAMENTO, idade);
}
...
void busca(void) {
        ...
        if (!checa_vazio(principal)) {
                ...
                fprintf(stdout, "Digite o EQUIPAMENTO para buscar: \\n--> ");
                scanf("%s", chave);
